...
namespace CyberArk.Extensions.Plugin.RealPowerShell
{
    public class Reconcile : BaseAction
    {
        ...
        override public int run(ref PlatformOutput platformOutput)
        {
            ...
            {
                log.WriteLine("reconcile", "customCode", "Unable to detect DEBUG setting in the platform settings - assume none", LogLevel.WARNING);
            }
            ...
            try
            {
                log.WriteLine("reconcile", "customCode", "Attempting to read the PowerShell script", LogLevel.INFO);
            ...
            {
                log.WriteLine("reconcile", "customCode", "Unable to read script file", LogLevel.ERROR);
            }
            ...
            try
            {
                ...
                log.WriteLine("reconcile", "customCode", "Checking if the Master account is populated to determine if this plugin is a Usage", LogLevel.INFO);
                ...
                log.WriteLine("reconcile", "customCode", "Confirmed plugin type = Usage", LogLevel.INFO);


            }
            catch
            {
                log.WriteLine("reconcile", "customCode", "Master is empty so this plugin is NOT a Usage", LogLevel.WARNING);
            }
            ...
            if (!isUsage)
            {
                ...
                log.WriteLine("reconcile", "customCode", "Confirmed plugin type = Non-usage", LogLevel.INFO);
                ...
                try
                {
                    ...
                    log.WriteLine("reconcile", "customCode", "Here are the properties on Target Account:\n" + KeyPairString + "\n", LogLevel.INFO);

                }
                catch
                {
                    //Console.WriteLine("Cought target");
                    log.WriteLine("reconcile", "customCode", "Account is not a Usage which means it must have a Target account yet does not.", LogLevel.ERROR);
                }
                ...
                try
                {
                    ...
                    log.WriteLine("reconcile", "customCode", "Here are the properties on Reconcile Account:\n" + KeyPairString + "\n", LogLevel.INFO);
                }
                catch
                {
                ...
                try
                {
                    ...
                    log.WriteLine("reconcile", "customCode", "Here are the properties on Logon Account:\n" + KeyPairString + "\n", LogLevel.INFO);
                }
                catch
                {
                    log.WriteLine("reconcile", "customCode", "Logon account is empty", LogLevel.WARNING);
                }
            } // if NOT a usage END
            ...
            try
            {
                ...
                powershellObject.Runspace.SessionStateProxy.SetVariable("CAOperation", "reconcile");
                ...
                {
                    log.WriteLine("reconcile", "PowerShellError", "PowerShell threw errors below--------------------------\n", LogLevel.ERROR);
                    ...
                    log.WriteLine("reconcile", "PowerShellError", "PowerShell raw message:\n\n" + sb.ToString() + "\n\n End of error \n", LogLevel.ERROR);
                    sb.Clear();
                ...
                else
                {
                    ...
                    foreach (PSObject rtnItem in PSresultsObject)
                    {
                        ...
                        {
                            log.WriteLine("reconcile", "PowerShellOutputSuccess", "We do indeed see 'PowerShell Success' in the PowerShell output so assume success!!!", LogLevel.INFO);
                            RC = 0;
                        }

                    }

                    log.WriteLine("reconcile", "PowerShellOutput", "\n-------------- Raw PowerShell Output Below --------------------------------------------------------------------\n\n" + noErrorOutPut + "\n\n--------------------------------------------------- End of Raw PowerShell Output Aboove  -------------------------------------------------------------", LogLevel.INFO);
                ...
                if (RC != 0)
                {
                    log.WriteLine("reconcile", "PowerShellOutput", "We did NOT see 'PowerShell Success' in the PowerShell output so assume FAILURE. Debug the PowerShell sript.", LogLevel.ERROR);
                }
