...
namespace CyberArk.Extensions.Plugin.RealPowerShell
{
    public class Verify : BaseAction
    {
        ...
        override public int run(ref PlatformOutput platformOutput)
        {
            ...
            {
                log.WriteLine("verify", "customCode", "Unable to detect DEBUG setting in the platform settings - assume none", LogLevel.WARNING);
            }
            ...
            try
            {
                log.WriteLine("verify", "customCode", "Attempting to read the PowerShell script", LogLevel.INFO);
            ...
            {
                log.WriteLine("verify", "customCode", "Unable to read script file", LogLevel.ERROR);
            }
            ...
            try
            {
                ...
                log.WriteLine("verify", "customCode", "Checking if the Master account is populated to determine if this plugin is a Usage", LogLevel.INFO);
                ...
                log.WriteLine("verify", "customCode", "Confirmed plugin type = Usage", LogLevel.INFO);


            }
            catch
            {
                log.WriteLine("verify", "customCode", "Master is empty so this plugin is NOT a Usage", LogLevel.WARNING);
            }
            ...
            if (!isUsage)
            {
                ...
                log.WriteLine("verify", "customCode", "Confirmed plugin type = Non-usage", LogLevel.INFO);
                ...
                try
                {
                    ...
                    log.WriteLine("verify", "customCode", "Here are the properties on Target Account:\n" + KeyPairString + "\n", LogLevel.INFO);

                }
                catch
                {
                    //Console.WriteLine("Cought target");
                    log.WriteLine("verify", "customCode", "Account is not a Usage which means it must have a Target account yet does not.", LogLevel.ERROR);
                }
                ...
                try
                {
                    ...
                    log.WriteLine("verify", "customCode", "Here are the properties on Reconcile Account:\n" + KeyPairString + "\n", LogLevel.INFO);
                }
                catch
                {
                ...
                try
                {
                    ...
                    log.WriteLine("verify", "customCode", "Here are the properties on Logon Account:\n" + KeyPairString + "\n", LogLevel.INFO);
                }
                catch
                {
                    log.WriteLine("verify", "customCode", "Logon account is empty", LogLevel.WARNING);
                }
            } // if NOT a usage END
            ...
            try
            {
                ...
                powershellObject.Runspace.SessionStateProxy.SetVariable("CAOperation", "verify");
                ...
                {
                    log.WriteLine("verify", "PowerShellError", "PowerShell threw errors below--------------------------------------------------------------------------\n", LogLevel.ERROR);
                    ...
                    log.WriteLine("verify", "PowerShellError", "PowerShell raw message:\n\n" + sb.ToString() + "\n\n End of error \n", LogLevel.ERROR);
                    sb.Clear();
                ...
                else
                {
                    ...
                    foreach (PSObject rtnItem in PSresultsObject)
                    {
                        ...
                        {
                            log.WriteLine("verify", "PowerShellOutputSuccess", "We do indeed see 'PowerShell Success' in the PowerShell output so assume success!!!", LogLevel.INFO);
                            RC = 0;
                        }
                ...
                if (RC != 0)
                {
                    log.WriteLine("verify", "PowerShellOutput", "We did NOT see 'PowerShell Success' in the PowerShell output so assume FAILURE. Debug the PowerShell sript.", LogLevel.ERROR);
                }
